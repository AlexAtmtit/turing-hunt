...
class GameSession {
    ...
    calculateResults() { /* ... */ const vC={}; const act=this.players.filter(p=>p.status==='active'); act.forEach(p=>{vC[p.id]=0;}); this.votes.forEach((vId,voterId)=>{const vr=this.players.find(p=>p.id===voterId);if(vId!==null&&vC.hasOwnProperty(vId)&&vr?.status==='active'){vC[vId]++;}}); console.log(`Vote counts:`,vC); let maxV=0; for(const pId in vC){if(vC[pId]>maxV)maxV=vC[pId];} let elimIds=[],elimN=[];if(maxV>0){for(const pId in vC){if(vC[pId]===maxV){elimIds.push(pId);const p=this.players.find(pl=>pl.id===pId);if(p)elimN.push(p.name);}}} if(elimIds.length>1){console.log(`Game ${this.id}: Tie detected (${elimIds.length} players). Randomly selecting one.`);const randomIndex=Math.floor(Math.random()*elimIds.length);const selectedId=elimIds[randomIndex];console.log(`Game ${this.id}: Randomly selected player ${selectedId} for elimination.`);elimIds=[selectedId];const selectedPlayer=this.players.find(pl=>pl.id===selectedId);elimN=selectedPlayer?[selectedPlayer.name]:[];} return{voteCounts:vC,eliminatedIds:elimIds,eliminatedNames:elimN}; }
